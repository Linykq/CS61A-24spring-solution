...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place_cur = self.place
        distance = 0
        while place_cur.entrance is not None:
            if distance > self.upper_bound:
                return None
            if not place_cur.is_hive:
                if len(place_cur.bees) > 0 and distance >= self.lower_bound:
                    return random_bee(place_cur.bees)
            place_cur = place_cur.entrance
            distance += 1
        return None
        # END Problem 3 and 4
    ...
    def action(self, gamestate):
        ...
        while place_cur:
            if place_cur.ant is not None:
                if not place_cur.ant.is_doubled:
                    place_cur.ant.double()
                    place_cur.ant.is_doubled = True
                if place_cur.ant.is_container and place_cur.ant.ant_contained is not None and not place_cur.ant.ant_contained.is_doubled:
                    place_cur.ant.ant_contained.double()
                    place_cur.ant.ant_contained.is_doubled = True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        place_cur = self.place
        distance = 0
        while not place_cur.is_hive:
            for bee in place_cur.bees:
                insects_dis[bee] = distance
            if place_cur.ant is not None and place_cur.ant is not self:
                insects_dis[place_cur.ant] = distance
                if place_cur.ant.is_container and place_cur.ant.ant_contained is not None and place_cur.ant.ant_contained is not self:
                    insects_dis[place_cur.ant.ant_contained] = distance
            place_cur = place_cur.entrance
            distance += 1
        return insects_dis
        # END Problem Optional 2
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage_dis = self.damage - 0.0625 *self.insects_shot - 0.25 * distance
        return damage_dis if damage_dis > 0 else 0
        # END Problem Optional 2
