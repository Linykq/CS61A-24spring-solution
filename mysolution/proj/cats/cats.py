...
def report_progress(typed, source, user_id, upload):
    ...
    while i < len(typed):
        if typed[i] != source[i]:
            d={'id': user_id, 'progress': float(i) / len(source)}
            upload(d)
            return float(i) / len(source)
        i += 1
    upload({'id': user_id, 'progress': float(i) / len(source)})
    return float(i) / len(source)
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    times = []
    match_now = match(words, times)
    for i in range(len(timestamps_per_player)):
        times_i = []
        for j in range(len(timestamps_per_player[i])-1):
            times_i.append(timestamps_per_player[i][j+1]-timestamps_per_player[i][j])
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fast_total=[[] for i in player_indices]
    for i in word_indices:
        fastplayer = 0
        fasttime = get_all_times(match)[0][i]
        for player in player_indices:
            if get_all_times(match)[player][i] < fasttime:
                fastplayer = player
                fasttime = get_all_times(match)[player][i]
        fast_total[fastplayer].append(get_all_words(match)[i])
    return fast_total
    # END PROBLEM 10
